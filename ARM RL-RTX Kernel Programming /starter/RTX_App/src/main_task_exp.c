...
/*--------------------------- task1 -----------------------------------*/
/* call rt_tsk_count_get() to find number of active tasks              */
/*---------------------------------------------------------------------*/
__task void task1(void)
{
      g_counter = rt_tsk_count_get();
      while(1) {
      printf("%\n");
      printf("out put of task1, count the active tasks: %d\t\n", g_counter);
...
__task void task2(void)
{
	...
	for(i = 0; i <g_counter; i++) { // this is a lazy way of doing loop.
...
/*--------------------------- task3 -----------------------------------*/
/* test task that calls the task one                        */
/*---------------------------------------------------------------------*/
__task void task3(void)
{
    while(1){
...
__task void init(void)
{
    ...
    g_tid = os_tsk_create(task4, 1);  /* task 3 at priority 1 */
	os_mut_wait(g_mut_uart, 0xFFFF);
	printf("init: created task2 with TID %d\n", g_tid);
	os_mut_release(g_mut_uart);
    
    g_tid = os_tsk_create(task5, 1);  /* task 3 at priority 1 */
	os_mut_wait(g_mut_uart, 0xFFFF);
	printf("init: created task2 with TID %d\n", g_tid);
	os_mut_release(g_mut_uart);
